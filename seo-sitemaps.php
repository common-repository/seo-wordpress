...
class ZEO_Sitemaps {
	...
	function __construct() {
		$options = get_option('mervin_sitemap');
		...
		add_action( 'zeo_hit_sitemap_index', array( $this, 'hit_sitemap_index' ) );

		// default stylesheet
		$this->stylesheet = '<?xml-stylesheet type="text/xsl" href="'.ZEO_FRONT_URL.'css/xml-sitemap-xsl.php"?>';
	}
	...
	function register_sitemap( $name, $function, $rewrite = '' ) {
		add_action( 'zeo_do_sitemap_' . $name, $function );
		if ( ! empty( $rewrite ) )
	...
	function build_sitemap( $type ) {
		$type = apply_filters('zeo_build_sitemap_post_type', $type);
		...
		else if ( has_action( 'zeo_do_sitemap_' . $type ) )
			do_action( 'zeo_do_sitemap_' . $type );
		else
	...
	function build_root_map() {
		...
		$this->sitemap .= apply_filters( 'zeo_sitemap_index', '' );
	...
	function build_post_type_map( $post_type ) {
		...
		$join_filter = '';
		$join_filter = apply_filters('zeo_typecount_join', $join_filter, $post_type);
		$where_filter = '';
		...
		while( $total > $offset ) {
			...
			foreach ( $posts as $p ) {
				...
				if ( zeo_get_value('meta-robots-noindex', $p->ID) && zeo_get_value('sitemap-include', $p->ID) != 'always' )
					continue;
				if ( zeo_get_value('sitemap-include', $p->ID) == 'never' )
					continue;
	...
	function build_tax_map( $taxonomy ) {
		...
		foreach( $terms as $c ) {
			...
			if ( zeo_get_term_meta( $c, $c->taxonomy, 'noindex' )
				&& zeo_get_term_meta( $c, $c->taxonomy, 'sitemap_include' ) != 'always' )
				continue;
	...
	function output() {
		...
		echo $this->sitemap;
		echo "\n" . '<!-- XML Sitemap generated by SEO WordPress -->';
